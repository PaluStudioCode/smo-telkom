...
/**
 * Form Request untuk Memperbarui Order Status.
 *
 * File ini menangani validasi dan otorisasi pada saat pengguna
 * memperbarui data Order Status yang sudah ada. Berbeda dengan Store,
 * request ini juga memeriksa kepemilikan data (hanya pemilik atau Super Admin
 * yang bisa mengedit) dan mengabaikan record saat ini pada validasi unique.
 *
...
class UpdateOrderStatusRequest extends FormRequest
{
     ...
     *
     * Otorisasi berlapis:
     * 1. Pengguna harus login dan data order_status harus ada
     * 2. Pengguna harus memiliki permission 'order_status.update'
     * 3. Pengguna harus Super Admin ATAU merupakan inputer yang memiliki data tersebut
     *    (inputer_id harus sama dengan ID pengguna yang login)
     *
     * @return bool
     */
     ...
     *
     * Logika sama dengan StoreOrderStatusRequest:
    ...
    public function rules(): array
    {
        ...
        return [
            // Nomor order unik per periode bulan, tapi mengabaikan record saat ini (ignore)
            // agar tidak gagal validasi saat nomor order tidak diubah
            'order_number' => [
                'required',
                'string',
                'max:100',
            ...
            // Status wajib sesuai daftar status yang valid dari model OrderStatus
            'status' => ['required', Rule::in(OrderStatus::statuses())],
            // Tahapan provisioning opsional
